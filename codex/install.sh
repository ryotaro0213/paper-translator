...
echo "[install] paper-translator root : ${ROOT_DIR}"
echo "[install] scripts directory     : ${ROOT_DIR}/scripts"
echo "[install] AGENTS.md source      : ${AGENTS_SRC}"
echo "[install] mode                  : ${MODE}"
echo "[install] (paths above are auto-detected from this script's location —"
echo "[install]  the repository can be cloned anywhere, folder name is arbitrary)"

write_configured_agents() {
  # Copy AGENTS.md to <target> while substituting $PAPER_TRANSLATOR_ROOT
  # occurrences with the absolute path. Also prepend a Configuration
  # header so Codex can see the resolved paths at a glance.
  local target="$1"
  local root="$2"
  {
    cat <<EOF
<!--
  Generated by paper-translator codex/install.sh
  Configured paths (Codex should use these literally, no env vars needed):
-->

## Configuration

- **PAPER_TRANSLATOR_ROOT**: \`${root}\`
- **Scripts directory**: \`${root}/scripts\`

When any step below says \`\$PAPER_TRANSLATOR_ROOT\` or \`\${PAPER_TRANSLATOR_ROOT}\`,
use the absolute path above.

---

EOF
    # Strip the first H1 line of the source to avoid duplicate top header.
    sed -e "s|\${PAPER_TRANSLATOR_ROOT}|${root}|g" \
        -e "s|\$PAPER_TRANSLATOR_ROOT|${root}|g" \
        "${AGENTS_SRC}"
  } > "${target}"
}
...
install_project() {
  ...
  write_configured_agents "${target}" "${ROOT_DIR}"
  echo "[ok] wrote AGENTS.md (paths baked in) -> ${target}"

  local env_file="$(pwd)/.paper-translator.env"
  cat > "${env_file}" <<EOF
# Optional — only needed if you want to run scripts manually from the terminal.
# Codex does NOT need you to source this; AGENTS.md has absolute paths baked in.
...
EOF
  ...
  echo "       No env setup is needed for Codex — AGENTS.md has the paths baked in."
}
...
install_global() {
  ...
  write_configured_agents "${instr}" "${ROOT_DIR}"
