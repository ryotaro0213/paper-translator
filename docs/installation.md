...
### 方法 C: シンボリックリンク（開発用）
```bash
ln -s /path/to/this/repo/plugins/paper-translator \
      ~/.claude/plugins/paper-translator
```
...
`view.sh` は Bash スクリプトなので Git Bash / WSL で動作します。Windows ネイティブ cmd では `view.bat` を別途用意するか、Python スクリプトを直接呼んでください。
