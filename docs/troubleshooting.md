...
```bash
PYTHONIOENCODING=utf-8 python scripts/validate_figures.py <out_dir>
```
...
4. 再実行: `python scripts/validate_figures.py <out_dir>`
...
### 出力先を変えたい
スラッシュコマンド内で `OUT=".paper-translator/outputs/<slug>"` としています。変更したい場合は `commands/translate-paper.md` を編集するか、Claude に「出力先は `~/Documents/papers/` にして」と指示。
